...
class MediaStoreRequestHandler extends ContentStreamRequestHandler {
  ...
  @Override public Result load(Request data) throws IOException {
    ...
    if (data.hasSize()) {
      PicassoKind picassoKind = getPicassoKind(data.targetWidth,
        data.resizeByMaxSide ? data.targetWidth : data.targetHeight);
      if (!isVideo && picassoKind == FULL) {
        return new Result(decodeContentStream(data), Picasso.LoadedFrom.DISK, exifOrientation);
      }
      ...
      if (bitmap != null) {
        return new Result(bitmap, Picasso.LoadedFrom.DISK, exifOrientation);
      }
    }
